...
We will first study the elastic $2 \to 2$ scattering of identical massive scalar particles, described by the scattering amplitude $\mathcal{M}(s,t,u)$. In this chapter, we will study how assuming consistency for the unknown UV completion of an EFT gives restrictions in the scattering amplitude at low energies.

\section{Properties of the Scattering Amplitude}

We start by reviewing the general properties that the scattering amplitude must satisfy if it originates from a consistent relativistic quantum theory. In particular, imposing unitarity, causality and crossing symmetry severely constrains the analytic structure of the amplitude in the complex Mandelstam plane. These properties will play a central role in the derivation of dispersion relations and, ultimately, in obtaining positivity constraints on the EFTs.

The unitarity of the $S$-matrix is expressed as
\begin{equation}
S^\dagger S = 1,
\end{equation}
 Writing the $S$-matrix as
\begin{equation}
S = 1 + i T,
\end{equation}
where $T$ is the transition operator, unitarity leads to
\begin{equation}
(1 - i T^\dagger)(1 + i T) = 1.
\end{equation}
...
Taking matrix elements between identical initial and final states $|i\rangle$ gives
\begin{equation}
\langle i | T - T^\dagger | i \rangle
= i \sum_n \langle i | T^\dagger | n \rangle \langle n | T | i \rangle,
\end{equation}
where we have inserted a complete set of intermediate states $\sum_n |n\rangle \langle n| = 1$.

The left-hand side can be written in terms of the scattering amplitude
$\mathcal{M}(i \to i) \equiv \langle i | T | i \rangle$ as
\begin{equation}
\langle i | T - T^\dagger | i \rangle
= \mathcal{M}(i \to i) - \mathcal{M}^*(i \to i)
= 2i\, \mathrm{Im}\,\mathcal{M}(i \to i).
\end{equation}

Thus, we obtain the optical theorem,
\begin{equation}
2\, \mathrm{Im}\,\mathcal{M}(i \to i)
= \sum_n |\mathcal{M}(i \to n)|^2.
\end{equation}
Therefore, the imaginary part of the scattering amplitude for identical external states must be positive:
\begin{equation}
\mathrm{Im}\,\mathcal{M}(i \to i)
= \frac{1}{2} \sum_n |\mathcal{M}(i \to n)|^2 \ge 0.
\end{equation}
...
In addition, causality demands the scattering amplitude to be a real analytic function of the Mandelstam variables \autocite{Adams:2006sv}. Real analyticity implies that the values of the amplitude above and below the real axis are related through the Schwarz reflection principle. 
...
For fixed momentum transfer $t$, the scattering amplitude can be regarded as a complex function of the Mandelstam variable $s$. The general requirements of Lorentz invariance, causality and unitarity that we just explored therefore imply that $\mathcal{M}(s,t)$ is an analytic function of $s$ in the complex plane, except for singularities with a direct physical interpretation.

These singularities consist of isolated poles and branch cuts. Poles correspond to the exchange of single-particle states, while branch cuts arise from multi-particle production thresholds. For the scattering of identical scalar particles of mass $m$, the first branch cut in the $s$-channel starts at the two-particle threshold,
\begin{equation}
    s = 4m^2,
\end{equation}
...
In an effective field theory, the amplitude is only expected to provide a valid description below a cutoff scale $\Lambda^2$, with $\Lambda^2 \geq 4m^2$. For the purposes of dispersion relations, the branch cuts associated with particle production are taken to lie above this cutoff, so that the low-energy expansion is valid away from the cuts. The typical analytical structure for an amplitude is shown in \Cref*{fig:estructura}.

\begin{figure}[hbtp]
    \begin{center}
        \includegraphics[width = 0.6 \textwidth]{../figures/base_estructura.png}
    \end{center}
    \caption{Typical analytical structure of the $2 \to 2$ scattering amplitude in an EFT. Pink lines represent branch cuts.}
    \label{fig:estructura}
\end{figure}

\subsection{Low-Energy Expansion}
The scattering amplitude admits a low-energy expansion that can be computed within an effective field theory valid below a cutoff scale. At tree level, and assuming that the EFT is weakly coupled up to the cutoff of the theory $\Lambda^2$, the amplitude can be written as the most general function symmetric under permutations of $s$, $t$ and $u$. In the case in which there is no exchange of other particles, the amplitude is
\begin{eqnarray}
...
At high energies, we can expand the scattering amplitude in partial waves as
\begin{equation}
    \mathcal{M}_{\text{high}}(s,t,u)
    =
...
\begin{equation}
    S_J(s) = 1 + i\, f_J(s).
\end{equation}
Unitarity of the $S$-matrix then implies that
\begin{equation}
    |S_J(s)| \leq 1.
\end{equation}
...
\end{equation}
the imaginary part of the high energy amplitude, which will play a central role in what follows, can be written as
\begin{equation}
    \Im \mathcal{M}_{\text{high}}(s,t,u)
    =
    \sum_{\substack{J \,\text{even}}}
    16\pi(2J+1)\,  \sqrt{ \frac{s}{s-4m^2} } \;
    \mathcal{P}_J\left(1+\frac{2t}{s-4m^2}\right)
    \rho_J(s).
...
\end{equation}
From now on we will make extensive use of the fact that the functions $\rho_J(s)$ are positive definite. We remark that this is a consequence of the unitarity of the $S$ matrix, and will remain true even if we do not know any more information about the amplitude at high energies. The upper bound is only saturated by strongly coupled theories.
...
\end{equation}
where $\mathcal{C}$ is the contour shown in \Cref*{fig:contours}.
\begin{figure}[hbtp]
    \begin{center}
        \includegraphics[width = 0.65 \textwidth]{../figures/contours.png}
    \end{center}
    \caption{Integration contour used to calculate the arcs. }
    \label{fig:contours}
\end{figure}
For simplicity, from now on we will define the crossing symmetric variable $\hat{s} = s-2m^2$.
The integrand is analytic inside the contour except for the poles on the real axis, that come both from the physical poles that are present in the amplitude and the ones that arise from the denominator. Thus, from Cauchy's residue theorem, we can write that 
\begin{equation}
...
From Cauchy's theorem, it is also possible to compute the coefficients \( a_n(t) \) by deforming the contour to $\mathcal{C}'$ and evaluating the contour integral explicitly. The result can be written as
\begin{equation}
    a_n(t)
    =
    \frac{1}{2\pi i}
...
\end{equation}
Froissart’s bound ensures that, for $n \geq 2$, 
$$
\lim_{\left| s \right| \to \infty} \frac{\mathcal{M}(s, t)}{s^2} = 0.
$$
As a consequence, the contributions from the portions of the contour taken to infinity vanish. In particular,
\begin{equation}
...
\begin{equation}
    \begin{split}
    a_n(t) = \sum_{J \text{ even}} \int_{\Lambda^2-2m^2}^{\infty} \frac{d\hat{s}}{\hat{s}} \, 32 \, (2 J + 1) \sqrt{\frac{\hat{s} + 2m^2}{\hat{s} - 2m^2}} \cdot \\
    \cdot \mathcal{P}_J \left( 1 + \frac{2t}{\hat{s}-2m^2} \right) \frac{2\hat{s}+t}{\hat{s}+t}\frac{\rho_J(\hat{s})}{[\hat{s}(\hat{s}+t)]^{n/2}}.
    \end{split}
    \end{equation}
...
\begin{equation}
    ...
    \sqrt{\frac{\hat{s} + 2m^2}{\hat{s} - 2m^2}}
    \right\rangle.
\end{equation}

To better appreciate the power of this result, we now compute the arcs explicitly for the EFT introduced in \Cref{eq:eft}. For simplicity, we focus on the $2 \rightarrow 2$ scattering of massless scalars. Using \Cref*{eq:residues}, we obtain \autocite{Caron-Huot:2020cmc}
\begin{equation}
...
null constraints. These are relations of the form
\begin{equation}
...
Moreover, null constraints play a crucial role in deriving bounds on Wilson coefficients. Because the Wilson coefficients themselves are given by positive 
...
For a massless scalar, crossing symmetry implies that
\begin{equation}
...
\label{eq:hihgenint}
\end{equation}
where $m, n \geq2$, the $t$-contour encircles the origin, and the $s$-contour encircles infinity. This integral projects out the coefficients in the low-energy expansion corresponding to the combination $s^m t^n - t^m s^n$.
...
Evaluating the integral using the residue theorem and projecting onto spin $J$ partial waves, we find the null constraints \autocite{Albert:2023jtd}
\begin{equation}
...
\end{equation}
Our next goal is to determine the range of values for the Wilson coefficients that are consistent with these constraints. To do this, we will rephrase our problem as a positive semidefinite optimization problem, which can be solved numerically.

As a concrete example, let us consider the problem of bounding the ratio
\[
\tilde{g}_n \equiv \frac{g_n}{g_2},
\]
where $g_n$ and $g_2$. Our goal is to find rigorous lower or upper bounds on $\tilde{g}_n$. We will follow the procedure shown in \autocite{Caron-Huot:2020cmc}.
...
\end{equation}
Maximizing $A + B \tilde{g}_3^0$, we find the optimal lower bound for $\tilde{g}_4$. A similar procedure can be done to find the optimal upper bound. \Cref*{fig:g3g4}, first found in \autocite{Caron-Huot:2020cmc}, shows the results found numerically for the allowed region of these two coefficients.
...
\section{Constraints on the Spectrum}
\label{sec:res}
Having established how to constrain the allowed parameter space of the Wilson coefficients in an EFT, we now turn to a complementary question. Suppose we extend the theory by allowing for the exchange of intermediate resonances with masses $m_i^2 < \Lambda^2$ and spin $J_i$. Positivity constraints restrict the local behaviour of the EFT amplitude, while the exchange of higher-spin states controls its high-energy growth. In particular, the exchange of a particle of spin $J$ typically induces contributions to the amplitude that grow with energy as $s^{J}$. 
This suggests that positivity bounds may, in turn, impose non-trivial constraints on the spectrum of higher-spin states that can be consistently coupled to the low-energy theory.
...
\end{equation}
with $\lambda_i^2 \geq 0$. Our goal is to derive an upper bound on $\lambda_i^2$. This is particularly useful, as it will provide further insight into the structure the spectrum in these theories. If the upper bound on $\lambda_i^2$ vanishes, it would imply that our scalar cannot couple to the $i$-th resonance respecting unitarity and positivity.
...
There is, however, a more elegant approach to this new problem. Instead of treating the resonance exchange explicitly within the low-energy amplitude and repeating the full dispersive analysis, we will regard the new poles introduced by the resonances as part of the high-energy contribution to the dispersion relations \autocite{Albert:2022oes}. In this framework, we use the fully analytic amplitude in \Cref{eq:eft}. The high-energy spectrum is then modeled as consisting of isolated poles together with a continuum that sets in above a scale $\Lambda'\.^2$, as shown in \Cref*{fig:hihgencont}.
\begin{figure}[hbtp]
    \begin{center}
        \includegraphics[width = 0.6 \textwidth]{../figures/highen_contour.png}
    \end{center}
    \caption{Modification of the contour used to study the coupling to resonances bellow the cutoff. The green lines represent the contour in which only the contributions above the cutoff enter the high energy average, while the black line represents the contour in which the poles coming from resonances are included in the high energy average.}
    \label{fig:hihgencont}
\end{figure}
...
Therefore, depending on the sign of $m_r^2 - 2m^2$, only one of these poles contributes to the high-energy average. The procedure described in the previous section applies without modification, the only required change being the replacement
\[
...
\section{Asymptotic UV behaviour}
...
Finding  dispersion relations with $n<2$ allows us to know under which IR conditions can the amplitude grow slower at high energies.
This situation is physically relevant in at least two contexts. 
First, there exist amplitudes that converge faster than required by the Froissart bound. A notable example is the $W^+ W^- \to W^+ W^-$ scattering amplitude, which we will analyse in detail in \Cref{chap:ww}. In the Standard Model, this amplitude grows as $s$ for big center-of-mass energy \autocite{Schwartz:2014sze}, so it satisfies once substracted dispersion relations.
