...
\label{chap:ww}
We now turn to the process $W^+ W^- \to W^+ W^-$. Our goal is to derive positivity constraints on departures from the Standard Model under different assumptions about the number of subtractions required in the dispersion relation. We are particularly interested in seeing what conditions allow for $\Lambda^2 \to \infty$.
...
m_Z^2 \geq 2 m^2.
\]
This bound already excludes the Standard Model configuration in the $\theta_{WE} \to 0$ limit, where \( m_Z^2 = m^2 \).
...
We now investigate how these bounds are modified in the presence of dimension-six operators. To this end, we fix both the $ZW^+W^-$ and Higgs--$W$ couplings to their Standard Model values, as well as $m_Z$. We consider the only three dimension-six operators that contribute to $WW$ scattering,
\begin{equation}
\mathcal{O}_H
=
\frac{1}{2}
\left[
\partial_\mu \left(H^\dagger H\right)
\right]^2,
\end{equation}
\begin{equation}
\mathcal{O}_{HW}
...
W^{c\,\rho\mu},
\end{equation}
which enter the amplitudes through the Wilson coefficients $c_H$, $c_{HW}$ and $c_{3W}$, respectively.
