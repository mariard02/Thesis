...
\begin{equation}
    \mathcal{M}_f(s) = - \lambda - \alpha^2 \left( - \frac{1}{M^2} + \frac{1}{-M^2 + s} + \frac{1}{-M^2 - s}  \right) + 2 g_2 s^2 + 4 g_4 s^4 + \cdots
\end{equation}
...
This shows that a theory of massless scalars satisfying  
...
\begin{equation}
    m^2 \geq \frac{M^2}{2},
\end{equation}
this is, the setup will be consistent as long as the mass of the resonance is small enough. 

The situation is more subtle in the case \(m=M\). Although the coefficient \(a_{-2}\) is positive, and therefore the simplest positivity constraint is satisfied, this does not necessarily imply the existence of a consistent UV completion. In particular, the null constraints associated with \((-2)\)-subtracted dispersion relations are expected to require the presence of additional higher-spin states. Since the exchange of a massive resonance contributes negatively to the relevant residue independently of its spin, these extra states would generate further negative contributions. Therefore, while the condition
\begin{equation}
    m^2 \geq \frac{M^2}{2}
\end{equation}
removes the immediate contradiction with positivity, it is unlikely that a fully consistent solution exists once all states required by analyticity, crossing symmetry and unitarity are taken into account. 
...
\begin{equation}
     \mathcal{M}(s, t)|_{s \to m_i^2} \sim  
    \frac{ -  \lambda_{J,n}^2\mathcal{P}_J \left( 1 + \frac{2t}{m^2} \right)}{s - m_n^2} 
    .
\end{equation}
Since 
\begin{equation}
...
\[
\lvert m_2^2 - 2m^2 \rvert \geq m^2 \Rightarrow m_2^2 \geq 3m^2 \text{ or } m_2^2 \leq m^2.
\]
...
$$
where $\lambda_j^2$ is the coupling of the spin $j$ resonance to the enternal scalar, $m_j$ its mass and $g_0$ the 4-point coupling. A spectrum inconsistency is equivalent to obtaining that $\lambda_j \to 0$ for a given mass.

We start by considering massless external legs, and the exchange of a spin 0 resonance. The upper bound for 
$$
...
Next, we consider the case of massive external legs with the exchange of a spin-2 resonance, shown in \Cref*{fig:massive_scalar_spin_2}. As in the previous scenario, the appearance of tails is a numerical artifact. This indicates, once again, that the resonance lies above the cutoff, so this scenario is not consistent, and is compatible with the analytical result.
\begin{figure}[hbtp]
    \centering
...
\begin{figure}[hbtp]
    ...
    \caption{Bounds for a massive spin 2 resonance and a massive spin 0 resonance with massless scalar external legs, shown for different values of the spin 0 resonance.}
    \label{fig:massive_scalar_spin_23}
\end{figure}

Finally, we explore the case of massive external legs with the exchange of both spin-2 and spin-0 resonances, where the mass of the spin-0 resonance is taken to be equal to that of the external legs.
...
\caption{Bounds for a massive spin 2  resonance and a massive spin 0 resonance with massive scalar external legs, shown for different values of the external mass. The spin 0 resonance and the external legs have equal mass. }
\label{fig:2res}
\end{figure}

\section*{Conclusions}

In this chapter, we have investigated the constraints imposed by positivity bounds on theories containing massive scalar resonances under different assumptions about the asymptotic behaviour of the scattering amplitude.

For amplitudes satisfying the standard Froissart behaviour, no non-trivial restrictions on the spectrum arise. The situation changes dramatically when stronger high-energy convergence conditions are imposed. In particular, for amplitudes admitting $-2$-subtracted dispersion relations, positivity of the dispersive representation excludes the exchange of massive resonances by massless scalars. For massive external states, consistency requires the resonance mass to satisfy $M^2 \leq 2m^2$.

We then focused on amplitudes obeying $0$-th subtracted dispersion relations. Using the analytic framework of \cite{Berman:2024kdh}, we showed that the state sitting at the mass gap must necessarily be a scalar. Consequently, the exchange of a higher-spin resonance, such as a spin-2 particle, requires the presence of a lighter scalar resonance. For massive external scalars, this argument leads to the condition
\[
m_2^2 \geq 3m^2
\qquad \text{or} \qquad
m_2^2 \leq m^2,
\]
when the scalar resonance has the same mass as the external particle.

These analytical conclusions were further explored numerically using the positivity-based optimization method introduced in \Cref{chap:positivity}. The numerical results confirm that isolated spin-0 resonances are allowed throughout the parameter space, while isolated spin-2 resonances are excluded below the cutoff. When both scalar and spin-2 resonances are present, consistent regions emerge precisely in the parameter ranges predicted analytically. Moreover, the numerical analysis systematically strengthens the analytic bounds.

Overall, our results demonstrate that stronger assumptions on the ultraviolet behaviour of scattering amplitudes lead to increasingly powerful constraints on the low-energy spectrum. In particular, unsubtracted dispersion relations impose a hierarchical structure on the spectrum, forcing scalar states to appear before higher-spin resonances. 