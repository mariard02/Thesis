...
\begin{Todo}
    Fixed $u$? (Longitudinals)
\end{Todo}

\section{Applications to $WW$ Scattering}
We now turn to the process $W^+ W^- \to W^+ W^-$. However, since our goal is to apply the methods introduced in \Cref*{sec:forwardlimit}, it is more convenient to consider instead the process $W^+ W^- \to W^- W^+$. In this configuration, the amplitude is invariant under $s \leftrightarrow u$ crossing, allowing us to avoid relying on additional scattering channels with different external states.
