...
For completness, it is also possible to do this same procedure to study $s \leftrightarrow u$ crosssing. For this case, the crossing matrix will be 
$$
X' = \begin{pmatrix}
    \frac{1}{3} & 1 & \frac{5}{3} \\
    \frac{1}{3} & \frac{1}{2} & -\frac{5}{6} \\
...
\subsection{Positive Odd-Substracted Sum Rules}
\label{sec:nopos}
...
\begin{equation}
    ...
    \rho_J^I(s).
\end{equation}
For the full amplitude to define a unitary $S$-matrix, each isospin channel must satisfy unitarity independently \autocite{Manohar:2008tc}. It then follows that the corresponding partial-wave absorptive coefficients are non-negative,
\begin{equation}
    \rho_J^I(s)\geq 0.
\end{equation}
...
\end{equation}
\begin{Todo}
    Corregir esto. Faltan partes imaginarias, cambiar el denominador...
\end{Todo}
...
\begin{pmatrix}
    ...
    \mathcal{M}_2
\end{pmatrix}.  $$
Thus, in order to construct a positive-definite sum rule with an odd number of substractions $n$, we must determine coefficients $a$, $b$, and $c$ such that all components of the vector
\begin{equation}
    \begin{pmatrix}
        a & b & c
    \end{pmatrix}
...
It is straightforward to verify that no choice of $a$, $b$, and $c$ yields a non-vanishing combination satisfying the non-negativity condition above. Therefore, when $n$ is odd, there are no non-trivial linear combinations of isospin amplitudes whose arc contributions are manifestly positive definite.
...
Another posibility is to go beyond the forward limit. Indeed, up to first order in $t$,
\begin{Todo}
    Escribir desarrollo
\end{Todo}
Thus, the first derivative of the arc is 
\begin{equation}
    \left. \frac{\partial a_n^I (t)}{\partial t} \right|_{t = 0} = ...
\end{equation}
\begin{Todo}
    Escribir esta ecuación
\end{Todo}
We can focus specifically in the factor that accompanies the crossed channel contribution, which is.
\begin{Todo}
    Ecuación
\end{Todo}
Because $l \geq 0$ and $s \geq \Lambda^2 > 4m^2$, 
\begin{Todo}
    Relación. Revisar también límites integral... no quiero liarla con esto.
\end{Todo}
\begin{Todo}
    Creo que tengo que demostrar
\end{Todo}
As we can see, the contribution from the crossed channel receives an extra minus sign, making it positive and making this equivalent to working with an even substracted arc. Thus,
\begin{equation}
    \left. \frac{\partial a_n (t)}{\partial t} \right|_{t = 0} \geq 0.
\end{equation}
...
\begin{table}[hbtp]
    ...
    \begin{tabular}{|c|c|>{\centering\arraybackslash}m{11.5cm}|}
        ...
        23 & $-$ & $\frac{1}{m^4}\,p_1\cdot\varepsilon_2\,(p_2\cdot\varepsilon_3\,p_2\cdot\varepsilon_4\,(p_2\cdot\varepsilon_1+p_3\cdot\varepsilon_1)-p_1\cdot\varepsilon_3\,(p_1\cdot\varepsilon_4\,(p_2\cdot\varepsilon_1+p_3\cdot\varepsilon_1)-p_2\cdot\varepsilon_4\,p_3\cdot\varepsilon_1))-(p_2\cdot\varepsilon_1\,(p_1\cdot\varepsilon_4\,(p_1\cdot\varepsilon_3+p_2\cdot\varepsilon_3)-p_2\cdot\varepsilon_3\,p_2\cdot\varepsilon_4)+(p_1\cdot\varepsilon_4\,p_2\cdot\varepsilon_3-p_1\cdot\varepsilon_3\,p_2\cdot\varepsilon_4)\,p_3\cdot\varepsilon_1)\,p_3\cdot\varepsilon_2$ \\ \hline 
...
It is important to emphasize that, although each subamplitude contains 25 structures, this does not imply that there are 75 independent structures in total. The reason is that some structures of different subamplitudes are related by crossing symmetry.

To see this explicitly, let us consider the following combination of structures in the subamplitude $\mathcal{M}_1(t,u)$:
\begin{equation}
E_{(1)2} + E_{(1)3} = \varepsilon_1 \cdot \varepsilon_3 \; \varepsilon_2 \cdot \varepsilon_4 \, .
\end{equation}

Now, we apply the crossing transformation $s \leftrightarrow u$. Under this operation, the subamplitude $\mathcal{M}_1(t,u)$ is mapped into $\mathcal{M}_2(s,t)$. Importantly, this transformation does not change the explicit form of the tensor structure, and we obtain
\begin{equation}
E_{(2)2} + E_{(2)3} = \varepsilon_1 \cdot \varepsilon_3 \; \varepsilon_2 \cdot \varepsilon_4 \, .
\end{equation}

Therefore, we arrive at the relation
\begin{equation}
E_{(1)2} + E_{(1)3} = E_{(2)2} + E_{(2)3} \, .
\end{equation}

This shows that these four structures are not independent: one of them can be written as a linear combination of the others. As a result, only three linearly independent combinations can be constructed from this set.

\section{Positivity Bounds in the Forward Limit}
\label{sec:forwardlimit}
...
\begin{equation}
    ...
    \mathcal{M}_{a \bar{d} c \bar{b}}^{\lambda'_1 \bar{\lambda'_2} \lambda'_3 \bar{\lambda'_4}}(s,t),
\end{equation}
...
\end{equation}
This expression shows that helicity-changing contributions ($\lambda_i \neq \lambda'_i$) are suppressed in the forward limit.

We can focus on elastic amplitudes of the form
\begin{equation}
    \mathcal{M}^{\lambda_1 \lambda_2 \lambda_1 \lambda_2}_{abcd} (u, t)
    \equiv \mathcal{M}^{\lambda_1 \lambda_2}_{abcd} (u, t),
\end{equation}
for which crossing implies
\begin{equation}
    \mathcal{M}^{\lambda_1 \lambda_2}_{abcd} (u, t)
    = \mathcal{M}^{\lambda_1 \bar{\lambda}_2}_{a \bar{d} c \bar{b}} (s, t)
    + \left( c\, \frac{\sqrt{|t|}\, m}{s} + \mathcal{O}\!\left(\frac{t m^2}{s u}\right) \right)
    \mathcal{M}(s,t)_{a \bar{d} c \bar{b}}^{\lambda'_1 \lambda'_2 \lambda'_3 \lambda'_4},
\end{equation}
...
\begin{equation}
    \mathcal{M}^{\lambda_1 \lambda_2}_{abcd} (u, t=0)
    = \mathcal{M}^{\lambda_1 \bar{\lambda}_2}_{a \bar{d} c \bar{b}} (s, t=0).
\end{equation}
...
\begin{equation}
    ...
    \frac{
        \mathcal{M}^{\lambda_1 \lambda_2}_{abcd}(s,t)
        + \mathcal{M}^{\lambda_1 \bar{\lambda}_2}_{a \bar{d} c \bar{b}}(s,t)
...
When $-t \ll \Lambda^2$, $m^2 \ll \Lambda^2$, or both, it can be shown \autocite{Bellazzini:2025shd} \autocite{Bellazzini:2023nqj} that  
\begin{equation}
    \frac{|a_n(t)|}{a_n(0)}
    \leq
    \frac{1}{\left(1+\frac{t/2}{\Lambda^2-2m^2}\right)^{n+1}}.
    \label{eq:trumpets}
\end{equation}

For sufficiently small values of $t/\Lambda^2$, the arcs can be approximated by their Taylor expansion around $t=0$,
\[
a_n(t)\simeq a_n(0)+mt,
\]
where $m$ denotes the slope. This corresponds to a straight line in the $(t,a_n)$ plane. The condition in \Cref*{eq:trumpets} then implies that this line must remain within the region bounded by
\[
\pm \frac{1}{\left(1+\frac{t/2}{\Lambda^2-2m^2}\right)^{n+1}}.
\]
Fixing a value of $t$ therefore yields both an upper and a lower bound on the slope $m$, as illustrated in \Cref*{fig:trumpets_bounds}.
\begin{figure}[hbtp]
\begin{center}
\includegraphics[width=0.85\textwidth]{../figures/trumpets_2sd.png}
\end{center}
\caption{Bounds on $a(t)$ for $m^2/\Lambda^2 = 0.1$, $-t/\Lambda^2 = 0.1$ and two substractions. The red region indicates the range of values of $a(t)/a_0$ consistent with \Cref*{eq:trumpets}. The upper boundary corresponds to the minimum allowed value of $m$, while the lower boundary corresponds to the maximum allowed value.}
\label{fig:trumpets_bounds}
\end{figure}

In particular, for a fixed $t_0$, the lower bound on $m$ is obtained from the straight line tangent to the upper curve at $t = 0$, namely
\begin{equation}
m_{\text{min}} = \frac{-1 - n}{2 (\Lambda^2 - 2 m^2)}.
\end{equation}
Similarly, the upper bound on $m$ corresponds to the slope of the straight line passing through $(0,1)$ and the lower curve evaluated at $t = t_0$, given by
\begin{equation}
m_{\text{max}} = -\frac{\left(\frac{t_0}{2 \Lambda^2 - 4 m^2}+1\right)^{-n-1}+1}{t_0}.
\end{equation}