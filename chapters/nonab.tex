...
\begin{equation}
    ...
    \rho_J^I(s).
\end{equation}
For the full amplitude to define a unitary $S$-matrix, each isospin channel must satisfy unitarity independently 
\begin{Todo}
    Citar 
\end{Todo}
. It then follows that the corresponding partial-wave absorptive coefficients are non-negative,
\begin{equation}
    \rho_J^I(s)\geq 0.
\end{equation}
...
In principle, we could now repeat the systematic analysis shown in \Cref{sec:sumrules} to derive general bounds on the Wilson coefficients and spectrum of these theories. This program is carried out in 
\begin{Todo}
    Citar piones 
\end{Todo}. In what follows, however, we will focus primarily on positivity constraints.
